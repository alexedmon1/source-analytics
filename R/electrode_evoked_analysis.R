#!/usr/bin/env Rscript
# electrode_evoked_analysis.R — Electrode-level evoked response statistical analysis
#
# Called by Python: Rscript R/electrode_evoked_analysis.R --data-dir ... --config ... --output-dir ...
#
# Reads electrode_evoked_measures.csv (scalar ITC/ERSP/STP values per subject/channel/measure),
...
# --- Argument parsing ---
parser <- ArgumentParser(description = "Electrode-level evoked response analysis (R)")
parser$add_argument("--data-dir", required = TRUE,
                    help = "Directory containing electrode_evoked_measures.csv")
...
measures_df <- read_csv(file.path(data_dir, "electrode_evoked_measures.csv"), show_col_types = FALSE)
message("  electrode_evoked_measures.csv: ", nrow(measures_df), " rows")
...
for (contrast in contrasts) {
  ...
  for (mname in measure_names) {
    ...
    n_channels <- length(unique(mdata$channel))

    mdata$group <- factor(mdata$group, levels = c(ga, gb))
    mdata$channel <- factor(mdata$channel)

    group_F <- NA; group_p <- NA
    channel_F <- NA; channel_p <- NA
    interaction_F <- NA; interaction_p <- NA
    converged <- TRUE; singular <- FALSE

    tryCatch({
      fit <- lmer(value ~ group * channel + (1 | subject), data = mdata)
      ...
      if ("channel" %in% rownames(aov)) {
        channel_F <- aov["channel", "F value"]
        channel_p <- aov["channel", "Pr(>F)"]
      }
    ...
    all_omnibus[[length(all_omnibus) + 1]] <- data.frame(
      ...
      channel_F = as.numeric(channel_F),
      channel_p = as.numeric(channel_p),
...
for (contrast in contrasts) {
  ...
  for (mname in measure_names) {
    ...
    tryCatch({
      ...
      emm <- emmeans(fit, pairwise ~ group | channel)
      ...
      for (i in seq_len(nrow(con_df))) {
        ch_name <- as.character(con_df$channel[i])

        emm_a <- emm_df %>%
          filter(channel == ch_name, group == ga) %>%
          pull(emmean)
        emm_b <- emm_df %>%
          filter(channel == ch_name, group == gb) %>%
          pull(emmean)

        # Explicitly compute estimate as ga - gb
...
if (nrow(posthoc_df) > 0) {
  ...
  message("  ", nrow(posthoc_df), " channel contrasts, ", sig_count, " significant")
}

# --- Region-level analysis (if electrode_categories defined) ---
...
if (length(electrode_categories) > 0) {
  ...
  region_df <- measures_df %>%
    inner_join(ch_to_region, by = "channel") %>%
...
# --- Global posthoc (marginal group comparisons averaged over channels) ---
...
if (nrow(global_posthoc_df) > 0) {
  ...
  write_csv(global_posthoc_df, file.path(tbl_dir, "electrode_evoked_posthoc_global.csv"))
  message("  Saved: tables/electrode_evoked_posthoc_global.csv")
...
# --- Export tables ---
message("\nExporting tables...")
for (f in c("electrode_evoked_omnibus.csv", "electrode_evoked_posthoc_channel.csv",
            "electrode_evoked_omnibus_region.csv", "electrode_evoked_posthoc_region.csv")) {
  fp <- file.path(tbl_dir, f)
  if (file.exists(fp)) file.remove(fp)
}

if (nrow(omnibus_df) > 0) {
  write_csv(omnibus_df, file.path(tbl_dir, "electrode_evoked_omnibus.csv"))
  message("  Saved: tables/electrode_evoked_omnibus.csv")
}
if (nrow(posthoc_df) > 0) {
  write_csv(posthoc_df, file.path(tbl_dir, "electrode_evoked_posthoc_channel.csv"))
  message("  Saved: tables/electrode_evoked_posthoc_channel.csv")
}
if (nrow(omnibus_region_df) > 0) {
  write_csv(omnibus_region_df, file.path(tbl_dir, "electrode_evoked_omnibus_region.csv"))
  message("  Saved: tables/electrode_evoked_omnibus_region.csv")
}
if (nrow(posthoc_region_df) > 0) {
  write_csv(posthoc_region_df, file.path(tbl_dir, "electrode_evoked_posthoc_region.csv"))
  message("  Saved: tables/electrode_evoked_posthoc_region.csv")
}

} else {
  message("Figures-only mode: loading existing tables...")
  omnibus_df <- tryCatch(read_csv(file.path(tbl_dir, "electrode_evoked_omnibus.csv"), show_col_types = FALSE), error = function(e) data.frame())
  posthoc_df <- tryCatch(read_csv(file.path(tbl_dir, "electrode_evoked_posthoc_channel.csv"), show_col_types = FALSE), error = function(e) data.frame())
  omnibus_region_df <- tryCatch(read_csv(file.path(tbl_dir, "electrode_evoked_omnibus_region.csv"), show_col_types = FALSE), error = function(e) data.frame())
  posthoc_region_df <- tryCatch(read_csv(file.path(tbl_dir, "electrode_evoked_posthoc_region.csv"), show_col_types = FALSE), error = function(e) data.frame())
  global_posthoc_df <- tryCatch(read_csv(file.path(tbl_dir, "electrode_evoked_posthoc_global.csv"), show_col_types = FALSE), error = function(e) data.frame())
}

# --- Figures ---
message("\nGenerating figures...")

# Group comparison violin/boxplots per measure
for (mname in measure_names) {
  ...
  # Aggregate to region means if electrode_categories present
  if (length(electrode_categories) > 0) {
    ch_to_region <- data.frame(
      channel = unlist(electrode_categories),
      region = rep(names(electrode_categories), lengths(electrode_categories)),
    ...
    plot_data$region <- factor(plot_data$region, levels = names(electrode_categories))
    ...
    p <- ggplot(plot_data, aes(x = region, y = value, fill = group)) +
      ...
      labs(title = paste0(mname, " by Region (Electrode)"),
           x = "Region", y = y_label, fill = "Group") +
      theme_minimal(base_size = 14) +
      theme(axis.text.x = element_text(angle = 45, hjust = 1))

    fname <- file.path(fig_dir, paste0("electrode_evoked_", mname, "_by_region.png"))
    ggsave(fname, p, width = 14, height = 6, dpi = 150)
    message("  Saved: ", basename(fname))
  }

  # Mean across all channels per subject — simple group comparison
  subj_means <- mdata %>%
  ...
  p2 <- ggplot(subj_means, aes(x = group_label, y = value, fill = group_label)) +
    ...
    labs(title = paste0(mname, " — Group Comparison (mean across channels)"),
         x = NULL, y = y_label) +
    theme_minimal(base_size = 14) +
  ...
  fname2 <- file.path(fig_dir, paste0("electrode_evoked_", mname, "_group.png"))
  ggsave(fname2, p2, width = 6, height = 5, dpi = 150)
  message("  Saved: ", basename(fname2))
}

# TF heatmap if electrode_evoked_tfr.csv exists
tfr_file <- file.path(data_dir, "electrode_evoked_tfr.csv")
  ...
  for (mtype in c("itc", "ersp", "stp")) {
    for (grp in group_order) {
      ...
      p <- ggplot(avg_data, aes(x = time, y = freq, fill = val)) +
        ...
        labs(title = paste0(toupper(mtype), " — ", grp, " (Electrode)"),
             x = "Time (s)", y = "Frequency (Hz)", fill = fill_label) +
        theme_minimal(base_size = 14)

      fname <- file.path(fig_dir, paste0("electrode_tfr_", mtype, "_", grp, ".png"))
...
if (nrow(posthoc_df) > 0) {
  ...
  if (nrow(sig_ph) > 0) {
    sig_ph$label <- paste0(sig_ph$contrast, " | ", sig_ph$channel)
    ...
    p <- ggplot(sig_ph, aes(x = hedges_g, y = reorder(label, hedges_g))) +
      ...
      labs(title = "Significant Post-Hoc Effects (Top 30, Electrode)",
           x = "Hedges' g", y = "", color = "Measure") +
      theme_minimal(base_size = 10)

    ggsave(file.path(fig_dir, "electrode_evoked_posthoc_forest.png"), p,
           width = 10, height = max(4, nrow(sig_ph) * 0.25), dpi = 150)
    message("  Saved: electrode_evoked_posthoc_forest.png")
  }
}
...
add("# Electrode Evoked Response Analysis \u2014 ", config$name)
add("")
...
add("**Analysis:** Electrode-Level Evoked Response (ITC, ERSP, STP)")
add("")
add("**Groups:** ", group_str)
add("")
add("**Measures:** ", paste(measure_names, collapse = ", "))
add("")
add("**Channels:** ", length(unique(measures_df$channel)))
add("")
...
add("**Statistics:** LMM (value ~ group * channel + (1|subject)), Type III ANOVA. ",
...
# Omnibus results
if (nrow(omnibus_df) > 0) {
  add("## Channel-Level Omnibus Results")
  add("")
...
# Post-hoc
if (nrow(posthoc_df) > 0) {
  add("## Post-Hoc Contrasts (Channel-Level)")
  add("")
  sig_ph <- posthoc_df %>% filter(significant == TRUE)
  if (nrow(sig_ph) > 0) {
    ...
    for (mname in unique(sig_ph$measure)) {
      ...
      add("")
      add("| Contrast | Channel | Estimate | SE | t | q | Hedges' g |")
      add("| --- | --- | --- | --- | --- | --- | --- |")
      for (i in seq_len(nrow(msig))) {
        row <- msig[i, ]
        add(sprintf("| %s | %s | %.4f | %.4f | %.2f | %.4f | %.2f |",
                    row$contrast, row$channel, row$estimate, row$SE,
