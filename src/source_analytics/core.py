...
ANALYSIS_METADATA: dict[str, dict[str, str]] = {
    ...
    "electrode_aperiodic":  {"category": "resting", "level": "electrode",  "domain": "Sensor-level",    "description": "Sensor-level aperiodic (1/f) analysis",
                             "about": "The aperiodic (1/f) exponent and offset at each scalp electrode, fit with specparam/FOOOF over 2-50 Hz -- the sensor-space counterpart of roi_aperiodic. Groups are compared per channel with a linear mixed model (dv ~ group * channel, subject as a random effect) plus a region-nested model (dv ~ group * region with (1|subject/channel), treating channels as replicates within scalp regions); effect sizes are Hedges' g and post-hoc p-values are Benjamini-Hochberg FDR-corrected across channels. Read it as: which electrodes/regions show a group shift in spectral slope (E/I proxy) or broadband offset, and in which direction (up = the first-listed group is higher)."},
    "electrode_comparison": {"category": "resting", "level": "electrode",  "domain": "Source vs Sensor", "display_name": "PSD",         "description": "Source vs electrode comparison",
    ...
    "fcd_comparison":       {"category": "resting", "level": "electrode",  "domain": "Source vs Sensor", "display_name": "Connectivity", "description": "Source vs sensor FCD comparison (mean + spatial CV)",
